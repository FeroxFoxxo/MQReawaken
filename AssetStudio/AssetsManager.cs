...
public class AssetsManager
{
    ...
    private readonly HashSet<string> noexistFiles = new(StringComparer.OrdinalIgnoreCase);
                ...
                foreach (var sharedFile in assetsFile.m_Externals)
                {
                    ...
                    if (!importFilesHash.Contains(sharedFileName))
                    {
                        ...
                        if (!noexistFiles.Contains(sharedFilePath))
                        {
                            ...
                            {
                                var findFiles = Directory.GetFiles(Path.GetDirectoryName(reader.FullPath),
                                    sharedFileName, SearchOption.AllDirectories);
    ...
    private void LoadAssetsFromMemory(FileReader reader, string originalPath, string unityVersion = null)
    {
        if (!assetsFileListHash.Contains(reader.FileName))
            ...
            {
                Logger.Error($"Error while reading assets file {reader.FullPath} from {Path.GetFileName(originalPath)}",
                    e);
            ...
            using (var archive = new ZipArchive(reader.BaseStream, ZipArchiveMode.Read))
            {
                ...
                foreach (var entry in archive.Entries)
                {
                    if (entry.Name.Contains(".split"))
                    {
                        ...
                        var basePath = Path.Combine(Path.GetDirectoryName(entry.FullName), baseName);
                ...
                foreach (var basePath in splitFiles)
                {
                    try
                    {
                        ...
                        var entryReader = new FileReader(basePath, splitStream);
                ...
                foreach (var entry in archive.Entries)
                {
                    try
                    {
                        var dummyPath = Path.Combine(Path.GetDirectoryName(reader.FullPath), reader.FileName,
                            entry.FullName);
                        // create a new stream
                        ...
                        var entryReader = new FileReader(dummyPath, streamReader);
                        LoadFile(entryReader);
